...
def createPitch(length,width, unity,linecolor):    # in meters
    ...
    if unity == "meters":
        ...
        if length <= 95:
            return "Didn't you mean meters as unity?"
        elif length >= 131 or width >= 101:
            return "Field dimensions are too big. Maximum length is 130, maximum width is 100"
        else:
            #Create figure
            fig=plt.figure()
            #fig.set_size_inches(7, 5)
